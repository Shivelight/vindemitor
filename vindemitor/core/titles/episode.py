...
from vindemitor.core.constants import AUDIO_CODEC_MAP, DYNAMIC_RANGE_MAP, VIDEO_CODEC_MAP
from vindemitor.core.titles.title import Title
from vindemitor.core.utilities import sanitize_filename
...
class Episode(Title):
    ...
    def __str__(self) -> str:
        return "{title} S{season:02}E{number:02} {name}".format(
            title=self.title, season=self.season, number=self.number, name=self.name or ""
        ).strip()
