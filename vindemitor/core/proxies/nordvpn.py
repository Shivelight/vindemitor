...
class NordVPN(Proxy):
    ...
    def get_recommended_servers(country_id: int) -> list[dict]:
        ...
        res = requests.get(
            ...
            params={"action": "servers_recommendations", "filters": json.dumps({"country_id": country_id})},
        )
        if not res.ok:
    ...
    def get_countries() -> list[dict]:
        ...
        res = requests.get(url="https://nordvpn.com/wp-admin/admin-ajax.php", params={"action": "servers_countries"})
        if not res.ok:
            raise ValueError(f"Failed to get a list of NordVPN countries [{res.status_code}]")
